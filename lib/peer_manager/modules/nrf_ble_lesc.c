...
{
	psa_status_t status;
	uint8_t pub_key[sizeof(uint8_t) + BLE_GAP_LESC_P256_PK_LEN];
	...
	} else if (pub_key_len != sizeof(pub_key)) {
		LOG_ERR("psa_export_public_key() unexpected output length %d", pub_key_len);
		return NRF_ERROR_INTERNAL;

	} else {
		/* The serialization format is expected to be 0x04 (uncompressed form). */
		__ASSERT(&pub_key[0] == ECC_PUB_KEY_UNCOMPRESSED_FORMAT_MARKER,
			 "Unexpected serialization format %#x", &pub_key[0]);
