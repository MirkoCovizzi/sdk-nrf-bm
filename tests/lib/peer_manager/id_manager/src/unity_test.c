...
void test_im_privacy_set(void)
{
	...
	__cmock_sd_ble_gap_privacy_set_ExpectWithArrayAndReturn(
...
void test_im_privacy_get(void)
{
	...
	__cmock_sd_ble_gap_privacy_get_ExpectWithArrayAndReturn(&privacy_params_out, 1,
								NRF_SUCCESS);
...
uint32_t pds_peer_data_read_stub(pm_peer_id_t peer_id, pm_peer_data_id_t data_id,
...
uint32_t pds_peer_data_read_stub_whitelist_get(pm_peer_id_t peer_id, pm_peer_data_id_t data_id,
					       pm_peer_data_t *p_data, uint32_t const *buf_size,
					       int cmock_num_calls)
{
...
uint32_t sd_ble_gap_device_identities_set_stub(ble_gap_id_key_t const *const *pp_id_keys,
...
void test_im_device_identities_list_set(void)
{
	...
	__cmock_pds_peer_data_read_ExpectAndReturn(peers[0], PM_PEER_DATA_ID_BONDING, NULL, NULL,
						   NRF_ERROR_NOT_FOUND);
	...
	for (int i = 0; i < 3; i++) {
		...
		__cmock_pds_peer_data_read_ExpectAndReturn(peers[i], PM_PEER_DATA_ID_BONDING, NULL,
							   NULL, NRF_SUCCESS);
		__cmock_pds_peer_data_read_IgnoreArg_p_data();
		__cmock_pds_peer_data_read_IgnoreArg_p_buf_len();
		__cmock_pds_peer_data_read_ReturnThruPtr_p_data(&mdata);
	}
	__cmock_sd_ble_gap_device_identities_set_StubWithCallback(
		sd_ble_gap_device_identities_set_stub);
	ret = im_device_identities_list_set(peers, 3);
	TEST_ASSERT_EQUAL(NRF_SUCCESS, ret);
}
