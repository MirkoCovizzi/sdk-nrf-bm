...
static im_connection_t m_connections_test[IM_MAX_CONN_HANDLES] = {
	{.peer_id = 5, .peer_address = {.addr = {4}}},
	{.peer_id = 4, .peer_address = {.addr = {3}}},
	{.peer_id = 3, .peer_address = {.addr = {2}}},
	{.peer_id = 2, .peer_address = {.addr = {1}}},
	{.peer_id = 1, .peer_address = {.addr = {0}}},
};
					       ...
					       int cmock_num_calls)
{
	...
	memcpy(return_im_evt_handler_bonding_data.peer_ble_id.id_addr_info.addr, m_addr,
	       BLE_GAP_ADDR_LEN);
	memcpy(return_im_evt_handler_bonding_data.peer_ble_id.id_info.irk, m_irk,
	       BLE_GAP_SEC_KEY_LEN);

	*p_peer_id = m_peer_id;
	p_peer_data->p_bonding_data =
		(pm_peer_data_bonding_t const *)&return_im_evt_handler_bonding_data;
...
void test_im_ble_evt_handler(void)
{
	...
	pds_peer_data_iterate_ReturnThruPtr_p_peer_id(&invalid_peer_id);
	pds_peer_data_iterate_IgnoreArg_p_data();
	...
	TEST_ASSERT_EQUAL_UINT(m_evt_handler_records[0 * IM_EVENT_HANDLERS_CNT].evt_id,
...
{
	pds_peer_data_iterate_prepare_Expect();
	pds_peer_data_iterate_ExpectAndReturn(PM_PEER_DATA_ID_BONDING, NULL, NULL, true);
	pds_peer_data_iterate_IgnoreArg_p_peer_id();
	pds_peer_data_iterate_IgnoreArg_p_data();
	pds_peer_data_iterate_ReturnThruPtr_p_peer_id(&m_peer_id);
	pds_peer_data_iterate_ReturnThruPtr_p_data((pm_peer_data_flash_t *)(p_peer_data));
	if (!expect_find) {
		pds_peer_data_iterate_ExpectAndReturn(PM_PEER_DATA_ID_BONDING, NULL, NULL, false);
		pds_peer_data_iterate_IgnoreArg_p_peer_id();
		pds_peer_data_iterate_IgnoreArg_p_data();
...
void test_duplicate_bonding_data(void)
{
	...
	ble_gap_addr_t addr_rpr = {.addr = {1, 1, 1, 1, 1, 1},
				   .addr_type = BLE_GAP_ADDR_TYPE_RANDOM_PRIVATE_RESOLVABLE};
	ble_gap_addr_t addr_rpnr = {.addr = {1, 1, 1, 1, 1, 2},
				    .addr_type = BLE_GAP_ADDR_TYPE_RANDOM_PRIVATE_NON_RESOLVABLE};
	ble_gap_addr_t addr_rs = {.addr = {1, 1, 1, 1, 1, 3},
...
void test_master_id_compare(void)
{
	...
	TEST_ASSERT_EQUAL_UINT(true,
			       im_master_ids_compare((ble_gap_master_id_t const *)&master_id1,
						     (ble_gap_master_id_t const *)&master_id2));
	TEST_ASSERT_EQUAL_UINT(false,
			       im_master_ids_compare((ble_gap_master_id_t const *)&master_id1,
						     (ble_gap_master_id_t const *)&master_id3));
	TEST_ASSERT_EQUAL_UINT(false,
			       im_master_ids_compare((ble_gap_master_id_t const *)&master_id1,
	...
	TEST_ASSERT_EQUAL_UINT(
		false, im_master_ids_compare((ble_gap_master_id_t const *)&empty_master_id1,
					     (ble_gap_master_id_t const *)&empty_master_id2));
}
							    ...
							    int cmock_num_calls)
{
	if (n_im_peer_id_get_by_master_id_callback_calls < 1) {
		...
		memcpy(return_im_peer_id_get_by_master_id_bonding_data.own_ltk.master_id.rand,
		       m_rand, BLE_GAP_SEC_RAND_LEN);
	} else {
	...
	p_peer_data->p_bonding_data =
		(pm_peer_data_bonding_t const *)&return_im_peer_id_get_by_master_id_bonding_data;
...
void test_im_master_id_is_valid(void)
{
	...
	TEST_ASSERT_EQUAL_UINT(
		true, im_master_id_is_valid((ble_gap_master_id_t const *)&valid_master_id));
	TEST_ASSERT_EQUAL_UINT(
						    ...
						    int cmock_num_calls)
{
	...
	m_bond_data.peer_ble_id.id_addr_info.addr[0] =
		m_addr[0] + m_pds_peer_data_iterate_calls_cnt;
...
{
	ret_code_t ret;
	ble_gap_addr_t public_addr = {.addr = {1, 2, 3, 4, 5, 6},
				      .addr_type = BLE_GAP_ADDR_TYPE_PUBLIC};

	sd_ble_gap_addr_set_ExpectAndReturn(&public_addr, NRF_SUCCESS);
...
void test_im_id_addr_get(void)
{
	...
	sd_ble_gap_addr_get_ExpectWithArrayAndReturn(&out_addr, 1, NRF_SUCCESS);
...
void test_im_privacy_set(void)
{
	...
	sd_ble_gap_privacy_set_ExpectWithArrayAndReturn(
		(ble_gap_privacy_params_t *)&privacy_params_on, 1, NRF_SUCCESS);
...
void test_im_privacy_get(void)
{
	...
	sd_ble_gap_privacy_get_ExpectWithArrayAndReturn(&privacy_params_out, 1, NRF_SUCCESS);

	ret = im_privacy_get(&privacy_params_out);
	TEST_ASSERT_EQUAL(NRF_SUCCESS, ret);
}

ret_code_t pds_peer_data_read_stub(pm_peer_id_t peer_id, pm_peer_data_id_t data_id,
				   pm_peer_data_t *p_data, uint32_t const *buf_size,
				   int cmock_num_calls)
{
	ble_gap_addr_t addr_public = {.addr_id_peer = 0,
				      .addr_type = BLE_GAP_ADDR_TYPE_PUBLIC,
				      .addr = {42, 1, 2, 3, 4, 5}};
	...
	case 4:
	case 5:
		memcpy(&p_data->p_bonding_data->peer_ble_id.id_addr_info, &addr_public,
		       sizeof(ble_gap_addr_t));
...
ret_code_t pds_peer_data_read_stub_whitelist_get(pm_peer_id_t peer_id, pm_peer_data_id_t data_id,
						 pm_peer_data_t *p_data, uint32_t const *buf_size,
						 int cmock_num_calls)
{
	...
	memcpy(&p_data->p_bonding_data->peer_ble_id.id_addr_info, &dummy_addr,
	       sizeof(ble_gap_addr_t));
...
void test_im_whitelist_set_clear(void)
{
	...
	sd_ble_gap_whitelist_set_ExpectAndReturn(NULL, 0, NRF_SUCCESS);
...
void test_im_whitelist_set(void)
{
	...
	sd_ble_gap_whitelist_set_ExpectAndReturn(NULL, peer_cnt, NRF_SUCCESS);
	sd_ble_gap_whitelist_set_IgnoreArg_pp_wl_addrs();
...
static pm_peer_data_bonding_t data[3] = {
	{.peer_ble_id = {.id_addr_info = {.addr = {1},
					  .addr_type = BLE_GAP_ADDR_TYPE_RANDOM_STATIC}}},
	{.peer_ble_id = {.id_addr_info = {.addr = {2},
					  .addr_type = BLE_GAP_ADDR_TYPE_RANDOM_STATIC}}},
	{.peer_ble_id = {
		 .id_addr_info = {.addr = {3}, .addr_type = BLE_GAP_ADDR_TYPE_RANDOM_STATIC}}}};

ret_code_t sd_ble_gap_device_identities_set_stub(ble_gap_id_key_t const *const *pp_id_keys,
						 ble_gap_irk_t const *const *pp_local_irks,
						 uint8_t len, int numCalls)
{
	...
	for (int i = 0; i < 3; i++) {
		TEST_ASSERT_EQUAL_MEMORY(&data[i].peer_ble_id, pp_id_keys[i],
					 sizeof(pm_peer_data_t));
	}
	return NRF_SUCCESS;
}
...
void test_im_device_identities_list_set(void)
{
	...
	sd_ble_gap_device_identities_set_ExpectAndReturn(NULL, NULL, 0, NRF_SUCCESS);
	ret = im_device_identities_list_set(NULL, BLE_GAP_DEVICE_IDENTITIES_MAX_COUNT);
	TEST_ASSERT_EQUAL(NRF_SUCCESS, ret);

	pds_peer_data_read_ExpectAndReturn(peers[0], PM_PEER_DATA_ID_BONDING, NULL, NULL,
					   NRF_ERROR_NOT_FOUND);
	pds_peer_data_read_IgnoreArg_p_data();
	pds_peer_data_read_IgnoreArg_p_buf_len();
	...
	for (int i = 0; i < 3; i++) {
		...
		pds_peer_data_read_IgnoreArg_p_data();
		pds_peer_data_read_IgnoreArg_p_buf_len();
		pds_peer_data_read_ReturnThruPtr_p_data(&mdata);
	}
	sd_ble_gap_device_identities_set_StubWithCallback(sd_ble_gap_device_identities_set_stub);
	ret = im_device_identities_list_set(peers, 3);
	TEST_ASSERT_EQUAL(NRF_SUCCESS, ret);
}
...
uint32_t callback(nrf_ecb_hal_data_t *p_ecb_data, int cmock_num_calls)
{
	for (uint32_t i = 0; i < SOC_ECB_KEY_LENGTH; i++) {
		TEST_ASSERT_EQUAL_UINT_MESSAGE(n_ecb_calls, cmock_num_calls,
					       "Unexpected callback received from cmock.");

		TEST_ASSERT_EQUAL_UINT_MESSAGE(expected_ecb_hal_data.key[i], p_ecb_data->key[i],
	...
	uint8_t hash_array[SOC_ECB_KEY_LENGTH] = {0x15, 0x9d, 0x5f, 0xb7, 0x2e, 0xbe, 0x23, 0x11,
...
void test_im_address_resolve(void)
{
	...
	sd_ecb_block_encrypt_StubWithCallback(callback);
	n_ecb_calls = 0;
